...
var pizzaSize = prompt("Enter the size of the pizza: S, M, or L", "S").toLowerCase(); // `toLowerCase()` will convert all input text into the prmopt to become lowercase. Therefore, it won't matter if the user inputs "S" or "s".
var perPizzaPrice;
var subTotalPrice = 0;
...
if (orderIngredients.length > 1) {
    ...
    for (var m = 1; m < orderIngredients.length; m++) {
        ...
        extraToppingsFee += xtraCharge; // per pizza
    }
}
